  ...
  return (
    <main style={styles.page}>
      <header style={styles.hero}>
        <p style={styles.env}>Environment: {"ENV_NAME"}</p>
        <h1 style={styles.title}>CTBus Games</h1>
        <p style={styles.subtitle}>Small experiments and playgrounds for browser-based games.</p>
      </header>
      <section style={styles.grid}>
        {games.map((game) => (
          <GameCard key={game.title} title={game.title} description={game.description} />
        ))}
      </section>
    </main>
  );
