...
<template>
  ...
  <v-container class="py-4 blog-content">
    ...
    <v-card class="my-4" variant="outlined"><pre class="code-block">
    <code >
    name: Tests
    on:
    pull_request:
    ...
    <p >To use this in your own project, you'll have to change the User identifier in the Resource spec <code>"arn:aws:iam::832242454463:role/*-ZappaLambdaExecutionRole"</code>. Once all that is working, the rest is pretty easy, just have to configure the AWS CLI and run the right zappa commands.</p>
    <code>
    .github/workflows/CICD.yml
    </code>
    <v-card class="my-4" variant="outlined"><pre class="code-block">
    ...
    <p >OK I went overboard and setup two WebDrivers: one for testing Chrome and one for testing Chrome on a mobile device. I also created this weird <code>arg</code> function so that I can use PyTest fixture parameterization to run the same tests on each of my WebDriver fixtures.</p>
    <code>
